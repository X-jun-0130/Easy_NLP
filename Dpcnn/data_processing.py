...
def process(filename, word_to_id, cat_to_id, max_length=300):
    ...
    x_pad = tf.keras.preprocessing.sequence.pad_sequences(data_id, max_length, padding='post', truncating='post')
    #y_pad = tf.keras.utils.to_categorical(label_id)
    return x_pad, np.array(label_id)
